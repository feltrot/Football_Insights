...
# Loop through the 18 categories
categories = df['Category_Column'].unique()  # Replace 'Category_Column' with the actual column name
for category in categories:
    # Filter data for the current category
    subset_data = df[df['Category_Column'] == category].copy()

    # Extract dependent variable (response variable) and independent variables
    y = subset_data['Dependent_Variable_Column']  # Replace 'Dependent_Variable_Column' with the actual column name
    X = subset_data.drop(['Dependent_Variable_Column', 'Category_Column'], axis=1)
    ...
    plt.ylabel('Explained Variance Ratio')
    plt.title(f'PCA - Category: {category}')
    plt.show()

    # Perform general linear model (GLM) regression
    X = sm.add_constant(X)  # Add a constant term to the independent variables
    model = sm.OLS(y, X).fit()

    # Display summary of the regression results
    print(f"Regression Results - Category: {category}")
    print(model.summary())

    # Plot the coefficients
    plt.figure(figsize=(10, 6))
    sns.barplot(x=X.columns, y=model.params)
    plt.xlabel('Independent Variables')
    plt.ylabel('Coefficients')
    plt.title(f'Regression Coefficients - Category: {category}')
